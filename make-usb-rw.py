#!/usr/bin/env python3
"""Create the deterministic writable FAT16 + VFAT test image for LatterOS."""
...
def main() -> None:
    output = Path(sys.argv[1] if len(sys.argv) > 1 else "latteros-usb-rw.img")
    ...
    system = (
        "Filesystem: FAT16 writable existing-file mode\r\n"
    ...
    print(
        f"Created {output} "
        f"({len(image) // (1024 * 1024)} MiB writable FAT16 + VFAT)"
    )
