#!/usr/bin/env python3
"""Create the complete complete writable FAT16 + VFAT test image for LatterOS."""
...
def main() -> None:
    output = Path(sys.argv[1] if len(sys.argv) > 1 else "latteros-usb-complete.img")
    ...
    system = (
        "Filesystem: FAT16 complete read-write mode\r\n"
    ...
    checklist = (
        ...
        "[x] Long filenames\r\n"
        "[x] USB SCSI WRITE(10)\r\n"
        "[x] Existing-file updates\r\n"
        "[x] File creation and deletion\r\n"
        "[x] Cluster allocation and resizing\r\n"
        "[x] Rename and move\r\n"
        "[x] Safe unmount\r\n"
        "[ ] AHCI and NVMe\r\n"
    ).encode("ascii")
    ...
    write_cluster(image, 10, write_test)

    output.write_bytes(image)
    print(
        f"Created {output} "
        f"({len(image) // (1024 * 1024)} MiB complete writable FAT16 + VFAT)"
    )
